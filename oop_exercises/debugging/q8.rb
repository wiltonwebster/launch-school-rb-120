...
=begin
We get a NoMethodError for select on nil, indicating our tasks variable is 
returning nil, rather than an array of tasks. We can see that we created a new
local variable tasks instead of invoking our setter method via self.tasks. 
However, we should not be using self.tasks either, since this will overwrite our
existing list of tasks. Instead, we create a new local variable called high
priority tasks, select from tasks, and then display this new array. Once we
display it, we discard it as we have no further use for it.
=end